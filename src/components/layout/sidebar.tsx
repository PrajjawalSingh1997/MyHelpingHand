...
import {
  ...
  Code2, Sparkles, Settings, ShieldCheck, Rocket, CheckSquare, Menu, X,
} from "lucide-react";
...
export function Sidebar({ modules, displayName, isAdmin }: SidebarProps) {
  ...
  return (
    <>
      {/* Mobile hamburger trigger — hidden on desktop, always reachable on mobile */}
      <button
        onClick={() => setOpen(true)}
        aria-label="Open menu"
        className="fixed left-3 top-3 z-50 flex h-9 w-9 items-center justify-center rounded-lg border border-[#2D2D3F] bg-[#0A0A0F] text-[#E2E8F0] md:hidden"
      >
        <Menu size={18} />
      </button>
      ...
      <aside
        className={cn(
          "fixed left-0 top-0 z-50 flex h-screen w-[240px] flex-col border-r border-[#2D2D3F] bg-[#0A0A0F]",
          "transition-transform duration-200 ease-out md:translate-x-0",
          open ? "translate-x-0" : "-translate-x-full"
        )}
      >
        {/* Logo */}
        <div className="flex items-center justify-between gap-2 border-b border-[#2D2D3F] px-5 py-4">
          ...
          <button
            ...
            className="text-[#64748B] hover:text-[#E2E8F0] md:hidden"
          >
            <X size={18} />
          </button>
        </div>
